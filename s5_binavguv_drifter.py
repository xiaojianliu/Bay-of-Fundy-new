...
#    yr+=(mo-1)//12;mo=(mo-1)%12+1; # this extends mo values beyond the formal range 1-12
    ...
    return RD 
FNs=[]
for filename in os.listdir(r'C:\Users\xiaojian\Desktop\Bay-of-Fundy-new-fig_4\Bay-of-Fundy-new-fig_4\driftfvcom_data4'):
...
for a in np.arange(len(th)):
    ...
    if th[a]>=RataDie(2004,4,1) and th[a]<=RataDie(2004,8,31):
        x.append(lonh[a])
        y.append(lath[a])
        uu.append(u[a])
        vv.append(v[a])
        romt.append(th[a])
    if th[a]>=RataDie(2005,4,1) and th[a]<=RataDie(2005,8,31):
        x.append(lonh[a])
        y.append(lath[a])
        uu.append(u[a])
        vv.append(v[a])
        romt.append(th[a])
    if th[a]>=RataDie(2006,4,1) and th[a]<=RataDie(2006,8,31):
        x.append(lonh[a])
        y.append(lath[a])
        uu.append(u[a])
        vv.append(v[a])
        romt.append(th[a])
    if th[a]>=RataDie(2007,4,1) and th[a]<=RataDie(2007,8,31):
        x.append(lonh[a])
        y.append(lath[a])
        uu.append(u[a])
        vv.append(v[a])
        romt.append(th[a])
    if th[a]>=RataDie(2008,4,1) and th[a]<=RataDie(2008,8,31):
        x.append(lonh[a])
        y.append(lath[a])
        uu.append(u[a])
        vv.append(v[a])
        romt.append(th[a])
    if th[a]>=RataDie(2009,4,1) and th[a]<=RataDie(2009,8,31):
        x.append(lonh[a])
        y.append(lath[a])
        uu.append(u[a])
        vv.append(v[a])
        romt.append(th[a])
    if th[a]>=RataDie(2010,4,1) and th[a]<=RataDie(2010,8,31):
        x.append(lonh[a])
        y.append(lath[a])
        uu.append(u[a])
        vv.append(v[a])
...
np.savez('binned_drifter12078.npz',xb=xb,yb=yb,ub_mean=ub_mean,ub_median=ub_median,ub_std=ub_std,ub_num=ub_num,vb_mean=vb_mean,vb_median=vb_median,vb_std=vb_std,vb_num=vb_num)
np.save('driftert',romt)

np.save('drifterlon',x)
np.save('drifterlat',y)
np.save('drifteru',uu)
np.save('drifterv',vv)